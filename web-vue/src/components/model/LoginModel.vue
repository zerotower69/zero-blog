  ...
  <v-dialog v-model="loginFlag" :fullscreen="isMobile" max-width="460">
    <v-card class="login-container" style="border-radius:4px">
      ...
      <div class="login-wrapper">
        ...
        <!--          TODO: 将此部分改成gitee github登录-->
        <div class="social-login-wrapper">
          <a
            class="iconfont icon-github"
            style="color: #000000;font-size: 30px;margin-right:40px;"
            @click="githubLogin"
          ></a>
          <a
  ...
  methods: {
    ...
    githubLogin() {
      ...
      window.location.replace(
        `${this.config.GITHUB_AUTHORIZE_URI}?client_id=${this.config.GITHUB_CLIENT_ID}
        &redirect_url=${this.config.GITHUB_REDIRECT_URI}`
      );
    },
    giteeLogin() {
      this.$store.commit("saveLoginUrl", this.$route.path);
