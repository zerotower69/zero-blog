...
export default {
  ...
  created() {
    ...
    //通过路径判断是微博登录还是qq登录
    if(that.$route.path==="/oauth/login/github"){
      if(that.$route.query.code){
        let param = new URLSearchParams();
        param.append("githubCode", that.$route.query.code);
        that.axios.post("/api/users/oauth/github", param).then(({ data }) => {
          if (data.flag) {
            //保存登录状态
    ...
    if (that.$route.path === "/oauth/login/gitee") {
      if (that.$route.query.code) {
        let param = new URLSearchParams();
        param.append("giteeCode", that.$route.query.code);
        that.axios.post("/api/users/oauth/gitee", param).then(({ data }) => {
          if (data.flag) {
            //保存登录状态
            that.$store.commit("login", data.data);
            that.$toast({ type: "success", message: data.message });
    ...
    if (this.$route.path === "/oauth/login/weibo") {
      //微博登录
      let param = new URLSearchParams();
      //将code传入后台
